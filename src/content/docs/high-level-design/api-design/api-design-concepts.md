...
**Tools that implement each one, and where it fits best**

| Algorithm | Used by / implemented in | Best enforced at — and why |
| --- | --- | --- |
| **Fixed window counter** | The Redis `INCR` + `EXPIRE` recipe (Redis's own rate-limiting cookbook example); GitHub's primary API rate limit (resets at a fixed timestamp each hour); most default configs in `django-ratelimit` / `express-rate-limit` | **Service / application**, for simple "N per hour/day" quotas where low storage cost matters more than boundary precision. Avoid it for anything security-sensitive (login, OTP) — the boundary trick is an exploitable doubling, not just a cosmetic inaccuracy |
| **Sliding window log** | The Redis sorted-set recipe (`ZADD` + `ZREMRANGEBYSCORE` + `ZCARD`) — Redis's canonical "exact" limiter pattern | **Service / application only**, and only for low-volume, high-value limits (login attempts, password resets, payment retries) where exactness is worth the per-request storage cost. Never at CDN/edge scale — storing a timestamp per request doesn't survive that volume |
| **Sliding window counter** | Cloudflare's rate limiting (publicly documented as a sliding-window approximation blending the current and previous fixed windows); Kong's rate-limiting plugin's "sliding window" policy | **CDN/edge or API gateway** — it's the algorithm edge providers reach for because it's nearly as cheap as a fixed window (two counters, not a log) but doesn't have the boundary-doubling flaw, which matters at the traffic volumes edge providers operate at |
| **Token bucket** | AWS API Gateway usage plans (explicitly token bucket: burst + steady rate); Stripe's API rate limits; Envoy's local rate limit filter; Kong's default policy; the GCRA variant used by Lyft's `ratelimit` service and Cloudflare (a cheap, single-timestamp token-bucket equivalent) | **API gateway**, as the general-purpose default for public APIs — it's the one place you want to deliberately allow legitimate bursty clients some slack while still capping the sustained rate, and gateways are where you already know the caller's identity/plan tier |
| **Leaky bucket** | NGINX's `ngx_http_limit_req_module` (NGINX's own docs describe it as leaky bucket); queue-based smoothing in front of a fragile downstream (SQS/RabbitMQ with a fixed-rate consumer) | **Reverse proxy / gateway**, for smoothing bursty client traffic into a steady stream a backend can actually handle — or as an **outbound** pattern in front of a downstream dependency you call, where the added queueing latency is more acceptable than it would be on a live inbound user request |
